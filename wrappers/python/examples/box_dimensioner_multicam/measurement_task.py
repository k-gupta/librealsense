 ...
 Calculate the cumulative pointcloud from the multiple devices
	...
	for (device, frame) in frames_devices.items():
		...
		if (length != 0 and width !=0 and height != 0):
			...
			print(bounding_box_points_device_upper)
			for i in range(len(bounding_box_points_device_upper)):
				if ( bounding_box_points_device_upper[i][0] > 0 and
				     bounding_box_points_device_upper[i][1] > 0 and
					 bounding_box_points_device_upper[i][0] < width and
					 bounding_box_points_device_upper[i][1] < height
				):
		...
		cv2.imwrite(f"{device}.png", color_image)
